...
@section('breadcrumb')
    ...
    <li><span class="text-primary font-medium">Edit Pengurus</span></li>
@endsection

@section('content')
    <div class="flex flex-col gap-3 mb-6">
        <h1 class="text-2xl font-bold font-serif text-base-content">Edit Pengurus</h1>
        <p class="text-xs text-base-content/60 font-sans">Perbarui data anggota pengurus: <strong class="text-base-content font-serif">{{ $boardMember->name }}</strong></p>
    </div>

    @include('admin.partials._form-errors')

    <form action="{{ route('admin.board-members.update', $boardMember) }}" 
          method="POST" 
            ...
            <div class="card bg-base-100 border border-base-300 shadow-sm">
                <div class="card-body p-6">
                    ...
                    <div class="form-control w-full md:w-1/2 mt-4">
                        ...
                        <input type="number" 
                               name="sort_order" 
                               value="{{ old('sort_order', $boardMember->sort_order) }}" 
                               min="0"
                               class="input input-bordered w-full" 
                               required />
        ...
        <div class="lg:col-span-4 flex flex-col gap-6">
            ...
            <div class="card bg-base-100 border border-base-300 shadow-sm">
                <div class="card-body p-6">
                    ...
                    <div class="form-control w-full">
                        <label class="label"><span class="label-text font-semibold">Ganti Foto Profil</span></label>
                        <input type="file" 
                               name="photo" 
                               accept="image/*"
                               @change="
                               ...
                               "
                               class="file-input file-input-bordered w-full file-input-primary file-input-sm" />
                        <span class="label-text-alt mt-1.5 text-base-content/50">Maks. 5MB (JPG, JPEG, PNG, WEBP)</span>
                    </div>

                    {{-- Foto Preview / Foto Saat Ini --}}
                        ...
                        <template x-if="!photoPreview">
                            <div class="text-center w-full">
                                ...
                                @if($boardMember->photo_url)
                                    <img src="{{ asset('storage/' . $boardMember->photo_url) }}" 
                                         alt="{{ $boardMember->name }}" 
                                         class="max-w-full max-h-56 rounded-btn object-cover shadow-sm mx-auto bg-base-300" />
                                @else
                                    <div class="text-center text-base-content/40 text-xs py-10 flex flex-col items-center gap-2">
                                        ...
                                        <span>Tidak ada foto saat ini (menggunakan avatar inisial).</span>
                                    </div>
                                @endif
                            </div>
                        </template>
            ...
            <div class="card bg-base-100 border border-base-300 shadow-sm text-xs">
                <div class="card-body p-4 text-base-content/70 space-y-2">
                    <div class="flex justify-between py-1 border-b border-base-200">
                        ...
                        <span class="font-semibold text-base-content">{{ $boardMember->created_at->format('d M Y, H:i') }}</span>
                    </div>
                    <div class="flex justify-between py-1">
                        <span>Pembaruan Terakhir:</span>
                        <span class="font-semibold text-base-content">{{ $boardMember->updated_at->format('d M Y, H:i') }}</span>
                    </div>
                </div>
            </div>
