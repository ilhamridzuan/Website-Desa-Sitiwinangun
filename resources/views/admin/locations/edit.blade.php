...
@section('title', 'Edit Lokasi Produksi')

@section('breadcrumb')
    <li><a href="{{ route('admin.locations.index') }}">Jelajah Desa</a></li>
    <li><span class="text-primary font-medium">Edit Lokasi</span></li>
@endsection

@section('content')
    <div class="flex flex-col gap-3 mb-6">
        <h1 class="text-2xl font-bold font-serif text-base-content">Edit Lokasi Rumah Produksi</h1>
        <p class="text-xs text-base-content/60 font-sans">Perbarui titik lokasi: <strong class="text-base-content font-serif">{{ $location->name }}</strong></p>
    </div>
    ...
    <form action="{{ route('admin.locations.update', $location) }}"
                        ...
                        <div class="form-control w-full md:col-span-2">
                            ...
                            <select name="artisan_id" class="select select-bordered w-full">
                                ...
                                @foreach($artisans as $artisan)
                                    <option value="{{ $artisan->id }}" {{ old('artisan_id', $location->artisan_id) == $artisan->id ? 'selected' : '' }}>
                                        {{ $artisan->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="form-control w-full md:col-span-2">
                            ...
                            <textarea name="main_products"
                                      ...
                                      class="textarea textarea-bordered h-20 w-full">{{ old('main_products', $location->main_products) }}</textarea>
                        </div>
                        ...
                        <div class="form-control w-full">
                            ...
                            <input type="text"
                                   name="visit_capacity"
                                   value="{{ old('visit_capacity', $location->visit_capacity) }}"
                        ...
                        <div class="form-control w-full md:col-span-2">
                            ...
                            <textarea name="edu_activities"
                                      ...
                                      class="textarea textarea-bordered h-20 w-full">{{ old('edu_activities', $location->edu_activities) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="lg:col-span-4 flex flex-col gap-6">
                    ...
                    <div class="form-control">
                        <label class="label cursor-pointer justify-start gap-3">
                            <input type="checkbox"
                                   ...
                                   {{ old('is_open_visit', $location->is_open_visit) ? 'checked' : '' }} />
            ...
            <div class="card bg-base-100 border border-base-300 shadow-sm"
                 x-data="{
                    imagePreview: {{ $location->photo_url ? "'" . asset('storage/' . $location->photo_url) . "'" : 'null' }}
                 }">
                <div class="card-body p-6">
                    <h3 class="card-title text-base font-bold font-serif mb-4 text-base-content border-b border-base-200 pb-2">
                        Foto Lokasi
                    ...
                    <div class="form-control w-full">
                        ...
                        <div class="mb-4 aspect-video rounded-btn bg-base-200 border border-base-300 overflow-hidden relative group flex items-center justify-center">
                            ...
                            @if($location->photo_url)
                                <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                    <span class="text-white text-xs font-medium px-2 py-1 bg-black/50 rounded">Foto Saat Ini</span>
                                </div>
                            @endif
                        </div>
                        ...
                        <input type="file"
                               ...
                               " />
                        <label class="label">
                            <span class="label-text-alt text-base-content/50">Kosongkan jika tidak ingin mengubah foto.</span>
                        </label>
                    </div>
                </div>
            </div>
